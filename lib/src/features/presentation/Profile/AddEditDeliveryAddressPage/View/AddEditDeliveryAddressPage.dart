...
class AddEditDeliveryAddressPage extends StatefulWidget {
  ...
  AddEditDeliveryAddressPage({ Key? key,
                               required this.isEditing,
                               required this.viewStateDelegate,
                               required this.deliveryAddressEntity }) : super(key: key);

  @override
  State<AddEditDeliveryAddressPage> createState() => _AddEditDeliveryAddressPageState();
}

class _AddEditDeliveryAddressPageState extends State<AddEditDeliveryAddressPage> with TextFormFieldDelegate, BaseView {
  String _actionText = "";
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FABRoundedRectangleView(text: 'Delete Delivery Address',
          backgroundColor: Colors.red,
          onPressed: () {
            _showAlertDeleteDeliveryAddress(context);
          },
          isHidden: !_isEditing()),
      appBar: createAppBarDone(title: _isEditing() ? "Edit Delivery Address" : "Add Delivery Address",
...
extension PrivateMethods on _AddEditDeliveryAddressPageState {
  ...
  _prepareView() {
    ...
    if(!_isEditing() && widget.deliveryAddressEntity == null) {
      widget.deliveryAddressEntity = DeliveryAddressEntity.getEmptyDeliveryAddress();
    }
  }
...
extension UserActions on _AddEditDeliveryAddressPageState {
  ...
  _editDeliveryAddress() {
    // Null Check
    if (widget.deliveryAddressEntity == null) { return; }
    
    (context).editDeliveryAddress(deliveryAddressEntity: widget.deliveryAddressEntity!).then((_) {
      setState(() {
  ...
  _addDeliveryAddress() {
    ...
    (context).addDeliveryAddress(deliveryAddressEntity: widget.deliveryAddressEntity!).then((_) {
      setState(() {
        (context).setLoadingState(isLoading: false);
      });
      _pop(context);
  ...
  _deleteDeliveryAddress() {
    ...
    (context).deleteDeliveryAddress(deliveryAddressEntity: widget.deliveryAddressEntity!).then((_) {
      setState(() {
        (context).setLoadingState(isLoading: false);
      });
      AlertView.closeAlertDialog(context);
